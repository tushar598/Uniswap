...
// Design tokens
//
// Every color used across the app funnels through this palette, and every
// translucent fill/border is derived from it with `withAlpha`, so a brand
// color only ever has to change in one place. Nothing below changes what
// renders — it's the same rgba() values the components used before, just
// computed instead of repeated as string literals throughout the file.
// ─────────────────────────────────────────────────────────────────────────────
const COLOR = {
  red: '#E84142',        // Avalanche brand red — primary accent
  redDark: '#C73536',
  cyan: '#00D4FF',        // secondary accent — AI / data surfaces
  cyanDark: '#0099CC',
  emerald: '#34D399',     // success / confirmed states
  amber: '#F5AC37',       // approvals / caution
  amberDark: '#E8940A',
  yellow: '#FACC15',      // medium-confidence badge
  danger: '#EF4444',      // destructive actions
  purple: '#7B2FBE',      // AI-commands stat accent
}
...
const PANEL_BG = 'linear-gradient(145deg, rgba(13,17,23,.97), rgba(22,30,46,.97))'

// Standard dark card shell — every panel (swap box, portfolio, activity, etc.)
// shares this same gradient body with a tinted border keyed to an accent color.
const panelStyle = (accent = COLOR.red, borderAlpha = 0.15) => ({
  background: PANEL_BG,
  border: `1px solid ${withAlpha(accent, borderAlpha)}`,
})

// Shared shell for the centered dialogs (Connect / Settings / Payment Link).
const modalOverlayStyle = { background: 'rgba(0,0,0,.88)', backdropFilter: 'blur(12px)' }
const modalCardStyle = (accent) => ({
  background: 'linear-gradient(145deg, #0D1117, #161E2E)',
  border: `1px solid ${withAlpha(accent, 0.2)}`,
  boxShadow: `0 0 100px ${withAlpha(accent, 0.1)}`,
})
...
const TokenSelector = ({ selected, onSelect, exclude }) => {
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen(!open)}
        className="flex items-center gap-2 px-3 py-2 rounded-2xl transition-all hover:scale-105 focus-visible:ring-1"
        style={{ background: 'rgba(255,255,255,.07)', border: '1px solid rgba(255,255,255,.1)' }}
      >
        <TokenIcon symbol={selected.symbol} color={selected.color} size="w-6 h-6" />
        <span className="font-bold text-white text-sm" style={{ fontFamily: FONT.display }}>{selected.symbol}</span>
        <span className="text-gray-500 text-xs ml-0.5">▾</span>
      </button>
      ...
      {open && (
        <>
          ...
          <div
            ref={menuRef}
            className="absolute top-full mt-2 right-0 z-50 rounded-2xl overflow-hidden w-52"
            style={{ background: '#0D1117', border: `1px solid ${withAlpha(COLOR.red, 0.2)}`, boxShadow: '0 20px 60px rgba(0,0,0,.8)' }}
          >
            {TOKENS.filter(t => t.symbol !== exclude?.symbol).map(token => (
              <button
                key={token.symbol}
...
const SwapInterface = ({ aiIntent, prices, onSwapComplete }) => {
  ...
  // Estimated output from prices when an on-chain quote isn't available yet
  const estimatedOut = (!amountOutFormatted && amountIn && pIn > 0 && pOut > 0)
    ? (parseFloat(amountIn) * pIn / pOut).toFixed(6)
    : null
  const displayOut = amountOutFormatted || estimatedOut || ''
  ...
  return (
    <div
      ...
      style={{ ...panelStyle(COLOR.red, 0.15), boxShadow: `0 0 40px ${withAlpha(COLOR.red, 0.04)}, inset 0 1px 0 rgba(255,255,255,.05)` }}
    >
      <div className="absolute -top-24 -right-24 w-48 h-48 rounded-full opacity-10 blur-3xl pointer-events-none" style={{ background: COLOR.red }} />

      {/* Header */}
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-2">
          <h3 className="text-white font-black text-lg tracking-widest" style={{ fontFamily: FONT.display }}>Swap</h3>
          <span
            className="text-xs px-2 py-0.5 rounded-full"
            style={{ background: withAlpha(COLOR.red, 0.1), color: COLOR.red, border: `1px solid ${withAlpha(COLOR.red, 0.2)}` }}
          >
            Trader Joe
          </span>
        </div>
          ...
          {['0.1', '0.5', '1.0'].map(s => {
            ...
            return (
              <button
                ...
                className="text-xs px-2.5 py-1 rounded-lg transition-all"
                style={{
                  background: active ? withAlpha(COLOR.red, 0.2) : 'rgba(255,255,255,.05)',
                  color: active ? COLOR.red : '#666',
                  border: active ? `1px solid ${withAlpha(COLOR.red, 0.4)}` : '1px solid transparent',
                }}
              >
                {s}%
              </button>
      ...
      {!isAvalanche && (
        <div
          className="mb-4 px-4 py-3 rounded-xl text-xs text-amber-300 flex items-center gap-2"
          style={{ background: 'rgba(250,204,21,.08)', border: '1px solid rgba(250,204,21,.2)' }}
        >
          <span>⚠️</span> Switch to Avalanche to enable swaps
        </div>
      )}
      ...
      <div className="rounded-2xl p-4 mb-1" style={{ background: 'rgba(255,255,255,.04)', border: '1px solid rgba(255,255,255,.06)' }}>
        <div className="flex justify-between text-xs text-gray-500 mb-3">
          <span className="tracking-wide">You pay</span>
          <span>{usdIn}</span>
        </div>
        <div className="flex items-center gap-3">
          <input
      ...
      <div className="flex justify-center my-2 relative z-10">
        <button
          ...
          className="w-9 h-9 rounded-xl flex items-center justify-center font-bold text-lg transition-colors"
          style={{ background: withAlpha(COLOR.red, 0.1), border: `1px solid ${withAlpha(COLOR.red, 0.25)}`, color: COLOR.red }}
        >
          ⇅
        </button>
      </div>
      ...
      <div className="rounded-2xl p-4 mb-4" style={{ background: 'rgba(255,255,255,.04)', border: '1px solid rgba(255,255,255,.06)' }}>
        <div className="flex justify-between text-xs text-gray-500 mb-3">
          <span className="tracking-wide">You receive</span>
          <span>{displayUsdOut}{estimatedOut ? ' (est.)' : ''}</span>
        </div>
        <div className="flex items-center gap-3">
          <div className="flex-1 text-white text-3xl font-black w-0 truncate" style={{ fontFamily: FONT.display }}>
            {displayOut || <span className="text-gray-800">0.0</span>}
          </div>
          <TokenSelector selected={tokenOut} onSelect={setTokenOut} exclude={tokenIn} />
        </div>
      ...
      {!isConnected ? (
        ...
        <button
          ...
          className="w-full py-4 rounded-2xl font-black text-sm tracking-widest uppercase transition-all hover:scale-[1.02] active:scale-[.98]"
          style={{
            background: approveLoading ? withAlpha(COLOR.amber, 0.1) : `linear-gradient(135deg, ${COLOR.amber}, ${COLOR.amberDark})`,
            color: approveLoading ? COLOR.amber : '#000',
        ...
        <button
          onClick={handleSwap}
          disabled={swapLoading || !amountIn || !isAvalanche}
          className="w-full py-4 rounded-2xl font-black text-sm tracking-widest uppercase transition-all hover:scale-[1.02] active:scale-[.98]"
          style={{
            background: (swapLoading || !amountIn || !isAvalanche) ? withAlpha(COLOR.red, 0.07) : `linear-gradient(135deg, ${COLOR.red}, ${COLOR.redDark})`,
            color: (swapLoading || !amountIn || !isAvalanche) ? withAlpha(COLOR.red, 0.2) : '#fff',
            fontFamily: FONT.display,
            boxShadow: (!swapLoading && amountIn && isAvalanche) ? `0 0 25px ${withAlpha(COLOR.red, 0.35)}` : 'none',
          }}
        >
          {swapLoading ? '⏳ Swapping…' : swapSuccess ? '✅ Swapped!' : 'Swap tokens'}
        </button>
...
const Portfolio = ({ address, prices }) => {
  ...
  return (
    <div ref={ref} className="rounded-3xl p-5 relative overflow-hidden" style={panelStyle(COLOR.red, 0.15)}>
      <div className="absolute -bottom-12 -left-12 w-36 h-36 rounded-full opacity-10 blur-3xl pointer-events-none" style={{ background: COLOR.red }} />
      <h3 className="text-white font-black text-base mb-4 tracking-widest" style={{ fontFamily: FONT.display }}>Portfolio</h3>
      <div className="space-y-0.5">
        {TOKENS.map((t, i) => <TokenBalanceRow key={t.symbol} token={t} address={address} prices={prices} index={i} />)}
      </div>
    </div>
...
const WalletInfoCard = () => {
  ...
  useEffect(() => {
    if (ref.current && isConnected) {
      gsap.fromTo(ref.current, { opacity: 0, scale: 0.94 }, { opacity: 1, scale: 1, duration: 0.6, ease: 'back.out(1.4)' })
    }
  }, [isConnected])
  ...
  return (
    <div
      ...
      style={{
        background: `linear-gradient(135deg, ${withAlpha(COLOR.red, 0.07)}, ${withAlpha(COLOR.cyan, 0.05)})`,
        border: `1px solid ${withAlpha(COLOR.red, 0.2)}`,
        boxShadow: `0 0 60px ${withAlpha(COLOR.red, 0.06)}`,
      }}
    >
      <div
        className="absolute inset-0 opacity-[.04] pointer-events-none"
        style={{
          backgroundImage: `linear-gradient(${withAlpha(COLOR.red, 0.6)} 1px, transparent 1px), linear-gradient(90deg, ${withAlpha(COLOR.red, 0.6)} 1px, transparent 1px)`,
      ...
      <div className="relative">
        ...
        <div className="mb-5">
          <p className="text-gray-500 text-xs uppercase tracking-widest mb-1">Balance</p>
          <div className="flex items-end gap-2">
            <span className="text-4xl font-black text-white" style={{ fontFamily: FONT.display }}>{bal}</span>
            <span className="text-gray-400 text-lg mb-0.5">{balance?.symbol || 'AVAX'}</span>
          </div>
        </div>
        ...
        <button
          onClick={handleCopy}
          className="w-full flex items-center justify-between px-4 py-2.5 rounded-xl mb-3 transition-colors hover:bg-white hover:bg-opacity-5"
          style={{ background: 'rgba(255,255,255,.04)', border: '1px solid rgba(255,255,255,.06)' }}
        >
          <span className="text-gray-500 text-xs tracking-wider">Address</span>
          <span className="flex items-center gap-2">
            <span className="text-white text-sm font-mono">{short}</span>
            <span className="text-xs" style={{ color: copied ? COLOR.red : '#555' }}>{copied ? '✓' : '⎘'}</span>
          </span>
        </button>
        ...
        >
          <span className="text-gray-500 text-xs tracking-wider">Snowtrace</span>
          <span className="text-xs" style={{ color: COLOR.red }}>View ↗</span>
        </a>

        <button
          onClick={disconnect}
          className="w-full py-2.5 rounded-xl font-bold text-xs tracking-widest uppercase transition-all hover:scale-[1.02] active:scale-[.98]"
          style={{ background: 'rgba(239,68,68,.1)', border: '1px solid rgba(239,68,68,.25)', color: COLOR.danger, fontFamily: FONT.display }}
        >
          Disconnect
        </button>
      </div>
...
const NetworkSwitcher = () => {
  ...
  return (
    <div ref={ref} className="rounded-3xl p-5" style={panelStyle(COLOR.red, 0.15)}>
      <h3 className="text-white font-black text-base mb-4 tracking-widest" style={{ fontFamily: FONT.display }}>Networks</h3>
      <div className="grid grid-cols-2 gap-2">
        {chains.map(chain => {
          const { color, name } = CHAIN_META[chain.id] || { color: '#888', name: chain.name }
          const active = chain.id === chainId
...
const ConnectModal = ({ onClose }) => {
  ...
  useEffect(() => {
    gsap.fromTo(ref.current, { opacity: 0, scale: 0.92, y: 24 }, { opacity: 1, scale: 1, y: 0, duration: 0.4, ease: 'back.out(1.6)' })
  }, [])

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={modalOverlayStyle}>
      <div ref={ref} className="w-full max-w-sm rounded-3xl p-7 relative overflow-hidden" style={modalCardStyle(COLOR.red)}>
        <div className="absolute -top-20 left-1/2 -translate-x-1/2 w-48 h-48 rounded-full opacity-20 blur-3xl pointer-events-none" style={{ background: COLOR.red }} />

        <div className="relative">
          <div className="flex items-center justify-between mb-2">
            <h2 className="text-white font-black text-2xl" style={{ fontFamily: FONT.display }}>Connect</h2>
            <button onClick={onClose} aria-label="Close" className="text-gray-600 hover:text-white text-2xl leading-none transition-colors">×</button>
          </div>
          <p className="text-gray-500 text-sm mb-6">Choose a wallet to connect to AgentSwap AI</p>
          ...
          <div className="space-y-2.5">
            {connectors.map(c => (
              <button
                ...
                className="w-full flex items-center gap-4 p-4 rounded-2xl transition-all hover:scale-[1.02] active:scale-[.98] group"
                style={{ background: 'rgba(255,255,255,.04)', border: '1px solid rgba(255,255,255,.07)' }}
                onMouseEnter={e => { e.currentTarget.style.borderColor = withAlpha(COLOR.red, 0.3) }}
                onMouseLeave={e => { e.currentTarget.style.borderColor = 'rgba(255,255,255,.07)' }}
              >
                {getWalletIcon(c.name)}
                <div className="text-left flex-1">
...
const SettingsModal = ({ onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={modalOverlayStyle}>
      <div ref={ref} className="w-full max-w-md rounded-3xl p-7 relative overflow-hidden" style={modalCardStyle(COLOR.cyan)}>
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-white font-black text-xl" style={{ fontFamily: FONT.display }}>⚙️ Settings</h2>
          <button onClick={onClose} aria-label="Close" className="text-gray-600 hover:text-white text-2xl leading-none transition-colors">×</button>
        </div>
        ...
        <div className="mb-4">
          ...
          <input
            ...
            onChange={e => setKey(e.target.value)}
            placeholder="AIza..."
            className="w-full bg-transparent text-white text-sm px-4 py-3 rounded-xl outline-none placeholder-gray-700 focus-visible:ring-1"
            style={{ border: '1px solid rgba(255,255,255,.1)', background: 'rgba(255,255,255,.03)' }}
          />
          <p className="text-gray-600 text-xs mt-2 leading-relaxed">
            Get your free API key at{' '}
            <a href="https://aistudio.google.com/apikey" target="_blank" rel="noopener noreferrer" className="underline" style={{ color: COLOR.cyan }}>
              Google AI Studio
            </a>
            . Your key is stored locally and never sent to our servers.
          </p>
        ...
        <button
          ...
          className="w-full py-3 rounded-xl font-bold text-sm tracking-widest uppercase transition-all hover:scale-[1.02]"
          style={{
            background: saved ? 'rgba(52,211,153,.2)' : `linear-gradient(135deg, ${COLOR.cyan}, ${COLOR.cyanDark})`,
            color: saved ? COLOR.emerald : '#000',
            fontFamily: FONT.display,
          }}
        >
          {saved ? '✓ Saved!' : 'Save key'}
...
const PaymentLinkModal = ({ onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={modalOverlayStyle}>
      <div ref={ref} className="w-full max-w-md rounded-3xl p-7 relative overflow-hidden" style={modalCardStyle(COLOR.red)}>
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-white font-black text-xl" style={{ fontFamily: FONT.display }}>🔗 Payment link</h2>
          <button onClick={onClose} aria-label="Close" className="text-gray-600 hover:text-white text-2xl leading-none transition-colors">×</button>
        </div>
        ...
        <div className="space-y-3 mb-5">
          <div>
            ...
            <input
              ...
              placeholder="10"
              className="w-full bg-transparent text-white text-lg px-4 py-3 rounded-xl outline-none placeholder-gray-700 focus-visible:ring-1"
              style={{ border: '1px solid rgba(255,255,255,.1)', background: 'rgba(255,255,255,.03)', fontFamily: FONT.display }}
            />
          </div>
          <div>
            <label className="text-gray-500 text-xs uppercase tracking-widest mb-1.5 block">Token</label>
              ...
              {['AVAX', 'USDC', 'USDT'].map(t => {
                ...
                return (
                  <button
                    ...
                    className="px-4 py-2 rounded-xl text-sm font-bold transition-all"
                    style={{
                      background: active ? withAlpha(COLOR.red, 0.15) : 'rgba(255,255,255,.04)',
                      border: `1px solid ${active ? withAlpha(COLOR.red, 0.3) : 'rgba(255,255,255,.06)'}`,
                      color: active ? COLOR.red : '#666',
                    }}
                  >
                    {t}
        ...
        <button
          ...
          className="w-full py-3 rounded-xl font-bold text-sm tracking-widest uppercase transition-all hover:scale-[1.02]"
          style={{
            background: copied ? 'rgba(52,211,153,.2)' : (!amount ? withAlpha(COLOR.red, 0.07) : `linear-gradient(135deg, ${COLOR.red}, ${COLOR.redDark})`),
            color: copied ? COLOR.emerald : (!amount ? withAlpha(COLOR.red, 0.2) : '#fff'),
            fontFamily: FONT.display,
          }}
        >
          {copied ? '✓ Copied!' : 'Copy payment link'}
  ...
  <div className="flex justify-start">
    <div className="rounded-2xl px-4 py-3" style={{ background: 'rgba(255,255,255,.04)', border: '1px solid rgba(255,255,255,.06)' }}>
      <div className="flex items-center gap-2">
        <div className="flex gap-1.5">
          {[0, 1, 2].map(i => (
            <div key={i} className="w-2 h-2 rounded-full" style={{ background: COLOR.cyan, animation: `thinking 1.4s ease-in-out ${i * 0.2}s infinite` }} />
          ))}
        </div>
        <span className="text-gray-500 text-xs ml-1">Analyzing…</span>
      </div>
...
const ChatMessage = ({ message, onExecute, onQuickAction, quickActions }) => {
  ...
  const confidenceColor = message.intent?.confidence > 0.8 ? COLOR.emerald : COLOR.yellow
  ...
  return (
    <div ref={ref} className={`flex ${isUser ? 'justify-end' : 'justify-start'}`}>
      <div
        ...
        style={{
          background: isUser ? `linear-gradient(135deg, ${withAlpha(COLOR.red, 0.15)}, ${withAlpha(COLOR.red, 0.08)})` : 'rgba(255,255,255,.04)',
          border: `1px solid ${isUser ? withAlpha(COLOR.red, 0.2) : 'rgba(255,255,255,.06)'}`,
        }}
      >
        {!isUser && (
          <div className="flex items-center gap-1.5 mb-2">
            <span className="text-xs">🤖</span>
            <span className="text-xs font-bold tracking-wider" style={{ color: COLOR.cyan }}>AgentSwap AI</span>
          </div>
        )}
        ...
        {/* Transaction Plan Card */}
        {message.type === 'action' && message.intent && (
          <div className="mt-3 rounded-xl p-4" style={{ background: withAlpha(COLOR.cyan, 0.05), border: `1px solid ${withAlpha(COLOR.cyan, 0.15)}` }}>
            <div className="flex items-center justify-between mb-3">
              <span className="text-xs font-bold tracking-wider uppercase" style={{ color: COLOR.cyan }}>
                {message.intent.action === 'swap' ? '🔄 Swap plan' : message.intent.action === 'send' ? '📤 Send plan' : '🔄 Convert plan'}
              </span>
              <span
                className="text-xs px-2 py-0.5 rounded-full"
                style={{ background: withAlpha(confidenceColor, 0.15), color: confidenceColor, border: `1px solid ${withAlpha(confidenceColor, 0.25)}` }}
              >
                {Math.round(message.intent.confidence * 100)}% match
              </span>
            </div>
            ...
            <div className="flex gap-2">
              <button
                onClick={onExecute}
                className="flex-1 py-2.5 rounded-lg text-xs font-bold tracking-wider uppercase transition-all hover:scale-[1.02]"
                style={{ background: `linear-gradient(135deg, ${COLOR.red}, ${COLOR.redDark})`, color: '#fff' }}
              >
                ⚡ Execute
              </button>
              <button
        ...
        {message.insights && message.insights.length > 0 && (
          <div className="mt-2.5 space-y-1.5">
            {message.insights.map((tip, i) => (
              <div key={i} className="flex items-start gap-2 text-xs text-gray-500">
                <span className="flex-shrink-0" style={{ color: COLOR.amber }}>💡</span>
                <span>{tip}</span>
              </div>
            ))}
          </div>
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {quickActions.map((action, i) => (
              <button
                ...
                className="text-xs px-3 py-1.5 rounded-full transition-all hover:scale-105 hover:bg-opacity-20"
                style={{ background: withAlpha(COLOR.red, 0.1), border: `1px solid ${withAlpha(COLOR.red, 0.2)}`, color: COLOR.red }}
              >
                {action}
              </button>
            ))}
...
const AIAssistantPanel = ({ onIntent, isConnected }) => {
  ...
  return (
    <div
      ...
      style={{ ...panelStyle(COLOR.cyan, 0.15), height: 'calc(100vh - 200px)', minHeight: '500px', maxHeight: '750px' }}
    >
      {/* Header */}
      <div className="px-5 py-4 flex items-center gap-3 flex-shrink-0" style={{ borderBottom: '1px solid rgba(255,255,255,.06)' }}>
        <div className="w-8 h-8 rounded-lg flex items-center justify-center text-sm" style={{ background: `linear-gradient(135deg, ${COLOR.red}, ${COLOR.cyan})` }}>
          🤖
        </div>
        <div>
          <h3 className="text-white font-bold text-sm tracking-wider" style={{ fontFamily: FONT.display }}>AI Assistant</h3>
          <p className="text-gray-600 text-xs">Powered by Gemini • Avalanche</p>
        </div>
        <div className="ml-auto flex items-center gap-1.5">
          <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4" style={{ scrollbarWidth: 'thin', scrollbarColor: 'rgba(0,212,255,.2) transparent' }}>
        {messages.map((msg, i) => (
          <ChatMessage
            key={i}
            message={msg}
            onExecute={() => msg.intent && onIntent(msg.intent)}
            onQuickAction={handleSend}
            quickActions={i === 0 ? quickActions : null}
          />
        ))}
      ...
      <div className="px-4 py-3 flex-shrink-0" style={{ borderTop: '1px solid rgba(255,255,255,.06)' }}>
        ...
        >
          ...
          <button
            ...
            className="w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold transition-all hover:scale-110 flex-shrink-0"
            style={{
              background: input.trim() ? `linear-gradient(135deg, ${COLOR.red}, ${COLOR.cyan})` : 'rgba(255,255,255,.05)',
              color: input.trim() ? '#fff' : '#333',
            }}
          >
            →
...
const AIInsightsBar = ({ tokenIn, tokenOut, amountIn }) => {
  ...
  if (amountIn && parseFloat(amountIn) > 1000) {
    insights.push({ icon: '⚠️', text: 'Large swap detected. Consider splitting into smaller transactions for better rates and less slippage.' })
  }
  if (tokenIn?.symbol === 'AVAX' || tokenOut?.symbol === 'AVAX') {
    insights.push({ icon: '⚡', text: 'AVAX swaps settle in under 2 seconds on Avalanche with minimal gas fees.' })
  }
  if (tokenOut?.symbol === 'USDC' || tokenOut?.symbol === 'USDT') {
    insights.push({ icon: '🛡️', text: 'Swapping to a stablecoin — great for locking in value during volatility.' })
  }
  insights.push({ icon: '🔀', text: 'Routing via Trader Joe V2.1 for best rates on Avalanche.' })

  return (
    <div className="rounded-2xl p-4" style={{ background: withAlpha(COLOR.cyan, 0.04), border: `1px solid ${withAlpha(COLOR.cyan, 0.1)}` }}>
      <h4 className="text-xs font-bold tracking-wider uppercase mb-2.5 flex items-center gap-2" style={{ color: COLOR.cyan, fontFamily: FONT.display }}>
        <span>💡</span> AI insights
      </h4>
      <div className="space-y-2">
        {insights.map((ins, i) => (
...
const DashboardStats = ({ txHistory }) => {
  ...
  const stats = [
    { label: 'Total transactions', value: totalTxns, icon: '📊', color: COLOR.red },
    { label: 'Volume swapped', value: totalSwapped > 0 ? totalSwapped.toFixed(2) : '0', icon: '💰', color: COLOR.cyan },
    { label: 'AI commands', value: aiCommands, icon: '🤖', color: COLOR.purple },
  ]

  return (
    <div className="grid grid-cols-3 gap-3">
      {stats.map((s, i) => (
        <div key={i} className="rounded-2xl p-4 text-center relative overflow-hidden" style={panelStyle(s.color, 0.13)}>
          <div className="text-2xl mb-1">{s.icon}</div>
          <div className="text-white font-black text-lg" style={{ fontFamily: FONT.display }}>{s.value}</div>
          <div className="text-gray-500 text-xs mt-0.5">{s.label}</div>
        </div>
      ))}
    </div>
...
const RecentActivity = ({ txHistory, chainId }) => {
  ...
  return (
    <div className="rounded-3xl p-5" style={panelStyle(COLOR.red, 0.15)}>
      <h3 className="text-white font-black text-base mb-3 tracking-widest" style={{ fontFamily: FONT.display }}>Activity</h3>
        ...
        <div className="space-y-2">
          {txHistory.slice(0, 8).map(tx => (
            <div key={tx.id} className="flex items-center gap-3 px-3 py-2.5 rounded-xl" style={{ background: 'rgba(255,255,255,.03)' }}>
              ...
              <span
                className="text-xs px-2 py-0.5 rounded-full"
                style={{ background: 'rgba(52,211,153,.1)', color: COLOR.emerald, border: '1px solid rgba(52,211,153,.2)' }}
              >
                {tx.status}
              </span>
              {tx.txHash && (
...
const StatsTicker = ({ prices }) => {
  ...
  const data = [
    { s: 'AVAX', key: 'avalanche-2', c: COLOR.red },
    { s: 'BTC', key: 'wrapped-bitcoin', c: '#F7931A' },
    { s: 'ETH', key: 'weth', c: '#627EEA' },
    { s: 'JOE', key: 'joe', c: '#E3507A' },
    { s: 'USDC', key: 'usd-coin', c: '#2775CA' },
  ...
  return (
    <div ref={ref} className="flex flex-wrap gap-2 justify-center mb-8">
      {data.map(({ s, key, c }) => {
        ...
        const ch = prices?.[key]?.usd_24h_change
        return (
          <div key={s} className="tick flex items-center gap-2 px-4 py-2 rounded-xl" style={{ background: 'rgba(255,255,255,.04)', border: '1px solid rgba(255,255,255,.06)' }}>
            <span className="w-2 h-2 rounded-full" style={{ background: c }} />
            <span className="text-gray-400 text-xs font-mono">{s}</span>
            <span className="text-white text-xs font-bold font-mono">{p ? `$${p.toLocaleString(undefined, { maximumFractionDigits: 2 })}` : '—'}</span>
            {ch != null && <span className={`text-xs font-mono ${ch >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>{ch >= 0 ? '+' : ''}{ch.toFixed(2)}%</span>}
...
const Hero = ({ onConnect }) => {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      gsap.to(orb1Ref.current, { x: 35, y: -25, duration: 6, repeat: -1, yoyo: true, ease: 'sine.inOut' })
      gsap.to(orb2Ref.current, { x: -30, y: 20, duration: 8, repeat: -1, yoyo: true, ease: 'sine.inOut', delay: 2 })
      gsap.fromTo(titleRef.current, { opacity: 0, y: 45, filter: 'blur(10px)' }, { opacity: 1, y: 0, filter: 'blur(0px)', duration: 1.2, ease: 'power3.out', delay: 0.2 })
      gsap.fromTo(subRef.current, { opacity: 0, y: 20 }, { opacity: 1, y: 0, duration: 0.8, ease: 'power2.out', delay: 0.7 })
      gsap.fromTo(btnRef.current, { opacity: 0, scale: 0.88 }, { opacity: 1, scale: 1, duration: 0.6, ease: 'back.out(2)', delay: 1 })
    }, heroRef)
    return () => ctx.revert()
  }, [])
  ...
  return (
    <div ref={heroRef} className="relative flex flex-col items-center justify-center text-center py-14 overflow-hidden">
      <div ref={orb1Ref} className="absolute top-0 left-1/4 w-80 h-80 rounded-full blur-3xl opacity-20 pointer-events-none" style={{ background: `radial-gradient(circle, ${COLOR.red}, transparent)` }} />
      <div ref={orb2Ref} className="absolute bottom-0 right-1/4 w-96 h-96 rounded-full blur-3xl opacity-15 pointer-events-none" style={{ background: `radial-gradient(circle, ${COLOR.cyan}, transparent)` }} />

      <div ref={titleRef} className="relative">
        <p className="text-xs font-medium tracking-[.4em] uppercase mb-3" style={{ color: COLOR.red, fontFamily: FONT.display }}>
          AI-powered DeFi on Avalanche
        </p>
        <h1
          ...
          style={{
            fontFamily: FONT.display,
            background: `linear-gradient(135deg, #fff 0%, ${COLOR.red} 50%, ${COLOR.cyan} 100%)`,
      ...
      <div ref={btnRef}>
        {!isConnected ? (
          <button
            onClick={onConnect}
            className="px-8 py-4 rounded-2xl font-black text-sm tracking-widest uppercase transition-all hover:scale-105 hover:brightness-110"
            style={{ background: `linear-gradient(135deg, ${COLOR.red}, ${COLOR.redDark})`, color: '#fff', fontFamily: FONT.display, boxShadow: `0 0 35px ${withAlpha(COLOR.red, 0.45)}` }}
          >
            Launch app →
          </button>
        ) : (
          <div className="flex items-center gap-2 px-5 py-3 rounded-2xl" style={{ background: withAlpha(COLOR.red, 0.1), border: `1px solid ${withAlpha(COLOR.red, 0.3)}` }}>
            <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
            <span className="text-emerald-400 text-sm font-medium tracking-wide">Wallet connected — scroll to dashboard</span>
          </div>
        )}
...
const FeatureCard = ({ icon, title, desc, index }) => {
  ...
  useEffect(() => {
    if (!ref.current) return
    gsap.fromTo(ref.current, { opacity: 0, y: 50 }, {
      opacity: 1, y: 0, duration: 0.6, delay: index * 0.15, ease: 'power3.out',
      scrollTrigger: { trigger: ref.current, start: 'top 90%', toggleActions: 'play none none reverse' },
    })
  }, [index])

  return (
    <div ref={ref} className="rounded-3xl p-6 relative overflow-hidden transition-all duration-300 hover:-translate-y-1.5" style={panelStyle(COLOR.red, 0.08)}>
      <div className="text-4xl mb-4">{icon}</div>
      <h3 className="text-white font-black text-base mb-2 tracking-wide" style={{ fontFamily: FONT.display }}>{title}</h3>
      <p className="text-gray-500 text-sm leading-relaxed">{desc}</p>
    </div>
  )
}
...
const HowItWorks = () => {
  ...
  return (
    <div ref={ref} className="mt-20 mb-16">
      <h2 className="text-center text-white font-black text-2xl mb-2 tracking-widest" style={{ fontFamily: FONT.display }}>How it works</h2>
      <p className="text-center text-gray-500 text-sm mb-10">Four simple steps to AI-powered DeFi</p>
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {steps.map((s, i) => (
          <div key={i} className="step rounded-2xl p-5 text-center relative" style={{ background: 'rgba(255,255,255,.03)', border: '1px solid rgba(255,255,255,.06)' }}>
            <div className="text-3xl mb-3">{s.icon}</div>
            <div className="text-xs font-bold tracking-widest mb-2" style={{ color: COLOR.red, fontFamily: FONT.display }}>Step {s.num}</div>
            <h4 className="text-white font-bold text-sm mb-1">{s.title}</h4>
            <p className="text-gray-500 text-xs leading-relaxed">{s.desc}</p>
            {i < 3 && <div className="hidden md:block absolute top-1/2 -right-3 text-gray-700 text-lg">→</div>}
          </div>
...
const WhyAvalanche = () => {
  ...
  return (
    <div
      ref={ref}
      className="mb-16 rounded-3xl p-8 relative overflow-hidden"
      style={{ background: `linear-gradient(135deg, ${withAlpha(COLOR.red, 0.06)}, ${withAlpha(COLOR.cyan, 0.04)})`, border: `1px solid ${withAlpha(COLOR.red, 0.15)}` }}
    >
      <div className="absolute -top-20 -right-20 w-60 h-60 rounded-full opacity-10 blur-3xl pointer-events-none" style={{ background: COLOR.red }} />
      <h2 className="text-white font-black text-2xl mb-2 tracking-widest" style={{ fontFamily: FONT.display }}>Why Avalanche?</h2>
      <p className="text-gray-500 text-sm mb-8">The fastest, cheapest, and most scalable blockchain for DeFi.</p>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {reasons.map((r, i) => (
          <div key={i} className="text-center p-4">
            <div className="text-3xl mb-2">{r.icon}</div>
            <div className="text-2xl font-black text-white mb-0.5" style={{ fontFamily: FONT.display }}>{r.stat}</div>
            <div className="text-xs font-bold tracking-wider uppercase mb-1" style={{ color: COLOR.red }}>{r.label}</div>
            <p className="text-gray-500 text-xs leading-relaxed">{r.desc}</p>
          </div>
        ))}
      </div>
...
const SupportedTokens = () => {
  ...
  useEffect(() => {
    if (!ref.current) return
    gsap.fromTo(ref.current.querySelectorAll('.token-card'), { opacity: 0, scale: 0.9 }, {
  ...
  return (
    <div ref={ref} className="mb-16">
      <h2 className="text-center text-white font-black text-2xl mb-2 tracking-widest" style={{ fontFamily: FONT.display }}>Supported tokens</h2>
      <p className="text-center text-gray-500 text-sm mb-8">Trade the top tokens on Avalanche</p>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {TOKENS.map(token => (
          <div
            key={token.symbol}
            className="token-card flex items-center gap-3 p-4 rounded-2xl transition-all hover:scale-[1.03]"
            style={{ background: 'rgba(255,255,255,.03)', border: `1px solid ${token.color}22` }}
          >
            <TokenIcon symbol={token.symbol} color={token.color} size="w-10 h-10" />
            <div>
...
const Dashboard = () => {
  ...
  return (
    <div className="min-h-screen text-white" style={{ background: 'radial-gradient(ellipse at top, #0a0e1a 0%, #050810 60%, #020409 100%)', fontFamily: FONT.mono }}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Orbitron:wght@400;700;900&family=Space+Mono:wght@400;700&display=swap');
        ...
        * { box-sizing: border-box; }
        ...
        input:focus-visible,
        a:focus-visible {
          outline: 2px solid rgba(232,65,66,.6);
          outline-offset: 2px;
        }
      ...
      <div className="max-w-7xl mx-auto px-4 pb-24">
        ...
        <nav className="flex items-center justify-between py-6 mb-2">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-lg flex items-center justify-center text-white font-black text-sm" style={{ background: `linear-gradient(135deg, ${COLOR.red}, ${COLOR.cyan})` }}>
              ⚡
            </div>
            <span
              className="font-black text-lg tracking-widest"
              style={{ fontFamily: FONT.display, background: `linear-gradient(90deg, ${COLOR.red}, ${COLOR.cyan})`, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}
            >
              AgentSwap
            </span>
          </div>
          ...
          <div className="flex items-center gap-3">
            {isConnected && (
              <button
                ...
                className="px-3 py-2 rounded-xl text-xs font-bold tracking-wider transition-all hover:scale-105"
                style={{ background: withAlpha(COLOR.red, 0.08), border: `1px solid ${withAlpha(COLOR.red, 0.15)}`, color: COLOR.red }}
              >
                🔗 Pay link
              </button>
            )}
            ...
            {!isConnected ? (
              <button
                ...
                className="px-5 py-2.5 rounded-xl font-black text-xs tracking-widest uppercase transition-all hover:scale-105"
                style={{ background: `linear-gradient(135deg, ${COLOR.red}, ${COLOR.redDark})`, color: '#fff', fontFamily: FONT.display, boxShadow: `0 0 20px ${withAlpha(COLOR.red, 0.3)}` }}
              >
                Connect wallet
              </button>
            ) : (
              <button
                onClick={() => setShowModal(true)}
                className="flex items-center gap-2 px-4 py-2.5 rounded-xl transition-all hover:scale-[1.02]"
                style={{ background: withAlpha(COLOR.red, 0.07), border: `1px solid ${withAlpha(COLOR.red, 0.2)}` }}
              >
                <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
                <span className="text-xs text-white font-mono">{address?.slice(0, 6)}…{address?.slice(-4)}</span>
              </button>
